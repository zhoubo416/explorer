...
1. **退出登录不清内存中的业务数据**：`ExploreStore.signOut()` 只清会话与错误状态，`goals` / `memories` 等仍在内存里；而 `loadRemoteData()` 只在远端结果非空时覆盖，`_applyCachedData()` 又会在内存非空时跳过。后果：同一台设备换账号登录，可能短暂（甚至在新账号无数据时持续）看到上一位用户的数据。删号路径已清掉本机磁盘缓存，但内存这条没动——因为 `ExploreStore.currentGoal` 在 `goals` 为空时会抛异常，要修得先把该 getter 改成可空并逐个处理 12 处调用点。
